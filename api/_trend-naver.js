...
async function fetchNaverSearchData(keyword){
  try{
    ...
    return {
      blogCount, newsCount, shopExists,
...
async function fetchNaverDatalab(keywords, period){
  try{
    ...
    // 각 키워드별 surgeRate, accel, durability 계산
    var result = {};
    data.results.forEach(function(r){
      var pts = r.data || [];
      if(pts.length < 4){ result[r.title] = null; return; }
      ...
      result[r.title] = { surgeRate:surge, accel:accel, durability:dur, rawData:pts };
    });
    return result;
  }catch(e){
...
async function fetchNaverBatch(keywords, period){
  ...
  for(var i=0; i<keywords.length; i++){
    var kw = keywords[i];
    results[kw] = {
      search:  await fetchNaverSearchData(kw),
    };
    await sleep(400);
  }
  // 데이터랩은 최대 5개씩 묶음
