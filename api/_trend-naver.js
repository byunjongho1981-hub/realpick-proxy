  ...
  return new Promise(function(resolve){
    try{
      ...
      var req=https.request({
        ...
        res.on('end',function(){
          if(done)return; done=true; clearTimeout(t);
          try{var d=JSON.parse(raw); resolve((d.items&&d.items[0])?d.items[0].slice(0,8).map(function(r){return r[0];}):[]);}
          catch(e){resolve([]);}
        });
      });
...
function calcSearchIntentFromData(keyword, naverData, suggestions){
  ...
  if(!naverData||naverData._fallback){
    ...
    return {type,score:Math.min(100,Math.max(0,Math.round(score))),buyRatio:br0,patterns:detected,suggestions:sugs};
  }
  ...
  if((naverData.recentNewsRatio||0)>=70)                   score+=8;
  else if((naverData.recentNewsRatio||0)>=40)              score+=4;

  var tot=Math.max(tBuy+tProb+tInfo,1);
  var buyRatio=type==='buy'?Math.max(50,Math.round(tBuy/tot*100)):Math.round(tBuy/tot*100);
  return {type,score:Math.min(100,Math.max(0,Math.round(score))),buyRatio,patterns:detected.slice(0,3),suggestions:sugs};
}
...
function parseDatalabResult(results, kwMap){
  ...
  (results||[]).forEach(function(r){
    ...
    console.log('[datalab parse]',kw,'surge:'+surge+'% accel:'+accel+'%');
  });
  return scores;
}
...
function parseDatalab(result, keyword){
  ...
  var mid=pts.slice(h),eh=mid.slice(0,Math.floor(mid.length/2)),rh=mid.slice(Math.floor(mid.length/2));
  var accel=avg(eh)>0?Math.round(((avg(rh)-avg(eh))/avg(eh))*100):0;
  var all=avg(pts),dur=Math.round((pts.filter(function(p){return p.ratio>=all;}).length/pts.length)*100);
...
async function fetchCategoryTopKeywords(catIds, period){
  ...
  for(var i=0;i<catIds.length;i++){
    ...
    console.log('[cat]',catId,'데이터랩 비교 시작 ('+seeds.length+'개)');

    var datalabScores=await compareKeywordsByDatalab(seeds,period);
    var ranked=seeds.slice().sort(function(a,b){return (datalabScores[b]||0)-(datalabScores[a]||0);});
    var top5=ranked.slice(0,5);
    console.log('[cat]',catId,'TOP5:',top5.map(function(kw){return kw+'('+(datalabScores[kw]||0)+'점)';}).join(' | '));
    ...
    for(var j=0;j<top5.length;j++){
      ...
      catItems.push({keyword:kw,catId:catId,insightData:insight&&!insight._fallback?insight:null,trendScore:(datalabScores[kw]||0)+insightScore});
      await sleep(150);
    }
    ...
    console.log('[cat]',catId,'최종:'+take+'개',catItems.slice(0,take).map(function(c){return c.keyword+'('+c.trendScore+'점)';}).join(' | '));
...
module.exports={
  ...
  buildKeywordClusters:function(keywords){
