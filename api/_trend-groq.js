...
// ── 5. ★ [9] 제외 후보 사유 설명 생성 ──────────────────────
// 노이즈·브랜드·뉴스성으로 제외된 키워드에 대해 이유 설명
async function generateExcludeReason(keywords){
  if(!keywords || !keywords.length) return {};
  var prompt =
    '아래 키워드들이 블루오션 제품 분석에서 제외되었다.\n'+
    '각각 왜 제외되었는지 한 줄(30자 이내)로 설명하라.\n'+
    'JSON: {"키워드":"제외이유", ...}\n\n'+
    '키워드 목록:\n'+keywords.join('\n');
  try{
    var resp   = await groqPost([
  ...
  }catch(e){ console.error('[groq-exclude]', e.message); }
  // 폴백: 규칙 기반 사유
  var result = {};
  keywords.forEach(function(kw){
    if(CFG.NOISE_PATTERNS.some(function(p){return p.test(kw);})){
      if(/정치|대통령|선거|국회/.test(kw)) result[kw] = '정치 이슈 키워드 — 제품 전환 불가';
      else if(/연예인|배우|가수|아이돌/.test(kw)) result[kw] = '연예인/방송 키워드 — 일시적 화제성';
      else if(/사고|사망|재난/.test(kw)) result[kw] = '사건·사고 키워드 — 제품 연결 없음';
      else if(/올림픽|월드컵|경기/.test(kw)) result[kw] = '스포츠 결과 키워드 — 단기 이슈';
      else result[kw] = '노이즈 키워드 — 수익화 어려움';
    } else if(CFG.BRAND_PATTERNS.some(function(p){return p.test(kw);})){
      result[kw] = '브랜드 키워드 — 일반 제품군 우선';
    } else {
      result[kw] = '제품 전환 적합성 낮음';
    }
  });
  return result;
}
