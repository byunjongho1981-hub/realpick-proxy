...
async function analyzeYouTube(keyword) {
  ...
  try {
    ...
    var topVideos = vids.slice(0,5).map(function(v) {
      ...
      return {
        id: v.id, title: sn2.title||'', channel: sn2.channelTitle||'',
        thumbnail: ((sn2.thumbnails&&(sn2.thumbnails.medium||sn2.thumbnails.default))||{}).url||'',
        views: sn(st.viewCount), likes: sn(st.likeCount),
        duration: dur, isShorts: dur<=60,
...
async function analyzeDatalab(keyword) {
  try {
    ...
    if (pts.length<4) return { surgeRate:0, surge3d:0, surge7d:0, trend:'unknown', avgRatio:0 };
    ...
    return {
      ...
      trend: surge7d>=30?'rising':surge7d>=10?'growing':surge7d>=-10?'stable':'falling',
      avgRatio: Math.round(ca*10)/10
    };
  } catch(e) { return { surgeRate:0, surge3d:0, surge7d:0, trend:'unknown', avgRatio:0 }; }
}
...
function judge(yt, blog, shop, dl) {
  ...
  // 타이밍 데이터
  var timingData = { surge3d: dl.surge3d||0, surge7d: dl.surge7d||0, trend: dl.trend||'unknown' };

  return { trendStatus, salesType, competition, timing, shortsfit, decision, competeReasons, timingData };
}
...
async function getGroqReason(kw, yt, blog, shop, dl, score, jdg) {
  ...
  try {
    var prompt =
      ...
      '예시: "네이버 쇼핑 상품 40개 + 경쟁 채널 많음 → 신규 진입 주의"\n'+
      '---\n'+
      '키워드: '+kw+'\n'+
      ...
      '검색량3일변화: '+(dl.surge3d>=0?'+':'')+dl.surge3d+'% | 7일변화: '+(dl.surge7d>=0?'+':'')+dl.surge7d+'%\n'+
      '트렌드: '+jdg.trendStatus+' | 경쟁도: '+jdg.competition+' | 타이밍: '+jdg.timing+' | 총점: '+score.total+'\n'+
      '---\n'+
      '출력: 추천 이유만 2줄 이내. 앞에 번호나 기호 붙이지 말 것.';
