...
async function classifyAndFilter(keywordItems){
  ...
  keywordItems.forEach(function(item){
    ...
    if(cls.isNoise || cls.type === CFG.KW_TYPE.NEWS_EVENT || cls.type === CFG.KW_TYPE.BRAND){
      excluded.push(item.keyword);
    } else {
  ...
  if(excluded.length){
    try{
      excludeReasons = await GROQ.generateExcludeReason(excluded);
  ...
  for(var i=0; i<filteredItems.length; i++){
    ...
    productNames.forEach(function(pname){
      ...
      candidates.push({
        ...
        hasVisualHook:      false,   // ★ [8]
        hasUsageScene:      false,   // ★ [8]
        isSeasonalFit:      false,
...
async function collectExternalData(candidates, period, apiStatus){
  ...
  candidates.forEach(function(c){
    ...
    if(c.ytData){
      ...
      c.hasVisualHook      = !!c.ytData.hasVisualHook;   // ★ [8]
      c.hasUsageScene      = !!c.ytData.hasUsageScene;   // ★ [8]
    }
...
async function generateDescriptions(candidates){
  ...
  for(var i=0;i<top10.length;i++){
    try{
      ...
      top10[i].groqReason = top10[i].productName+'의 상승 트렌드 확인됨. 즉시 콘텐츠 선점 필요.';
...
module.exports = async function(req, res){
  ...
  req.on('end', async function(){
    try{
      ...
      if(!filtered.length)
        return res.status(200).json({candidates:[],apiStatus:apiStatus,excluded:excluded,excludeReasons:excludeReasons,error:'유효 키워드 없음'});

      var candidates = await expandToProducts(filtered);
      if(!candidates.length)
        return res.status(200).json({candidates:[],apiStatus:apiStatus,excluded:excluded,excludeReasons:excludeReasons,error:'제품 후보 없음'});
      ...
      return res.status(200).json({
        ...
        groups:          groups,
        actionPriority:  actionPriority,    // ★ [13] 표3
        excluded:        excluded,
        excludeReasons:  excludeReasons,    // ★ [9]
