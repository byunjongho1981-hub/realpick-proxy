    ...
    <div style={{ fontFamily:"'Segoe UI',sans-serif" }}>
      ...
      {activeTab==="home" && (
        <div>
          <div style={{ display:"grid", gridTemplateColumns:"1fr 1fr", gap:10, marginBottom:16 }}>
            <button onClick={()=>{ setActiveTab("realtime"); handleRun("realtime"); }} disabled={loadingRT} style={{ padding:"14px 10px", borderRadius:11, border:"none", cursor:loadingRT?"not-allowed":"pointer", background:loadingRT?"#222":"linear-gradient(135deg,#ff4400,#ffd700)", color:loadingRT?"#555":"#000", fontWeight:800, fontSize:13, display:"flex", alignItems:"center", justifyContent:"center", gap:6 }}>
              {loadingRT ? <><Spinner color="#ff8800" size={14}/> 분석 중...</> : <>⚡ 실시간 추천</>}
            </button>
            <button onClick={()=>{ setActiveTab("stable"); handleRun("stable"); }} disabled={loadingST} style={{ padding:"14px 10px", borderRadius:11, border:"none", cursor:loadingST?"not-allowed":"pointer", background:loadingST?"#222":"linear-gradient(135deg,#0055ff,#03c75a)", color:loadingST?"#555":"#fff", fontWeight:800, fontSize:13, display:"flex", alignItems:"center", justifyContent:"center", gap:6 }}>
          ...
          <div style={{ display:"grid", gridTemplateColumns:"repeat(2,1fr)", gap:8, marginBottom:14 }}>
            <div style={{ background:"rgba(255,68,0,0.05)", border:"1px solid rgba(255,68,0,0.15)", borderRadius:9, padding:"10px 12px" }}>
              ...
              <div style={{ fontSize:10, color:"#555", marginTop:4 }}>속도 35% · DataLab 25% · 쇼핑클릭 20%</div>
            </div>
            <div style={{ background:"rgba(3,199,90,0.05)", border:"1px solid rgba(3,199,90,0.15)", borderRadius:9, padding:"10px 12px" }}>
              ...
              <div style={{ fontSize:11, color:"#777" }}>쇼핑·검색 종합 데이터 기반</div>
        ...
        <div>
          <div style={{ display:"flex", alignItems:"center", justifyContent:"space-between", marginBottom:12 }}>
            <div>
              ...
              <div style={{ fontSize:10, color:"#555", marginTop:2 }}>최근 12~24h 급상승 상품</div>
            </div>
          ...
          {!loadingRT && rtResults.length>0 && (
            <div>
              ...
              <div style={{ fontSize:11, color:"#555", marginBottom:10 }}>급상승 키워드 <b style={{ color:"#ffd700" }}>TOP {rtResults.length}</b></div>
        ...
        <div>
          ...
          {!loadingST && stResults.length>0 && (
            <div>
              ...
              <div style={{ fontSize:11, color:"#555", marginBottom:10 }}>안정 추천 <b style={{ color:"#03c75a" }}>TOP {stResults.length}</b></div>
              <div style={{ display:"flex", flexDirection:"column", gap:7 }}>
      ...
      {activeTab==="cat" && (
        <div>
          ...
          <div style={{ fontSize:10, color:"#555", marginBottom:12 }}>{ranRT?"실시간 추천 기준":ranST?"안정 추천 기준":"먼저 추천 분석을 실행해주세요"}</div>
          {!ranRT && !ranST ? (
      ...
      {activeTab==="setting" && (
        <div style={{ display:"flex", flexDirection:"column", gap:10 }}>
          ...
          <div style={{ background:"rgba(255,255,255,0.02)", border:"1px solid rgba(255,255,255,0.06)", borderRadius:11, padding:"14px" }}>
            ...
            {[
              { label:"⚡ 실시간", rt:"YT속도35% + DataLab25% + 쇼핑클릭20% + 구매의도15% + 경쟁역수5%" },
              { label:"💰 안정",   rt:"트렌드25% + YT25% + 구매의도25% + 쇼핑20% + 경쟁역수10%" },
            ].map((r,i)=>(
              <div key={i} style={{ padding:"7px 0", borderBottom:i<1?"1px solid rgba(255,255,255,0.04)":"none" }}>
                <div style={{ fontSize:11, color:"#ccc", marginBottom:2 }}>{r.label}</div>
                <div style={{ fontSize:10, color:"#555" }}>{r.rt}</div>
              </div>
            ))}
          </div>
        </div>
