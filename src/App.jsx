...
const REVENUE = {
  coupang: { clicks: 1240, conversions: 38, revenue: 47200 },
  naver:   { clicks: 890,  conversions: 22, revenue: 29800 },
};
...
const ALL_SOURCES = [
  ...
  { name: "Google Trends 쇼핑", url: "https://trends.google.com/trends/trendingsearches/daily/rss?geo=KR&category=18" },
  { name: "뽐뿌 핫딜",          url: "https://www.ppomppu.co.kr/rss.php?id=ppomppu" },
  { name: "에펨코리아",         url: "https://www.fmkorea.com/rss/best" },
  { name: "클리앙 알뜰구매",    url: "https://www.clien.net/service/rss" },
];

function parseRSS(xml, src) {
  const out = [];
  const cdata = [...xml.matchAll(/<title><!\[CDATA\[(.*?)\]\]><\/title>/g)];
  if (cdata.length > 1) { cdata.slice(1,20).forEach(m => m[1]?.trim() && out.push({ keyword: m[1].trim(), source: src })); return out; }
  [...xml.matchAll(/<title>(.*?)<\/title>/g)].slice(1,20).forEach(m => {
    const t = m[1].replace(/<[^>]+>/g,"").trim();
    if (t) out.push({ keyword: t, source: src });
  });
  return out;
}

async function fetchRSS(src, onHealth) {
  const proxies = [
  ...
  for (const p of proxies) {
    try {
      const r = await fetch(p(src.url), { signal: AbortSignal.timeout(6000) });
      ...
      if (items.length > 0) { onHealth(src.name, "alive", items.length); return items; }
    } catch {}
  }
  onHealth(src.name, "dead", 0);
  return [];
}
...
export default function App() {
  ...
  function updateHealth(name, status, count) {
    setSourceHealth(prev => ({ ...prev, [name]: { status, count } }));
  }
  ...
  async function hunt(force = false) {
    ...
    setSourceHealth({}); setStatusMsg("📡 소스 생존 확인 중...");
    const initH = {}; ALL_SOURCES.forEach(s => { initH[s.name] = { status:"checking", count:0 }; }); setSourceHealth(initH);

    let collected = [];
    const alive = [];
    const results = await Promise.all(ALL_SOURCES.map(src => fetchRSS(src, updateHealth)));
    results.forEach((items, i) => {
      if (items.length > 0) { alive.push(ALL_SOURCES[i].name); collected = [...collected, ...items]; }
    });

    setStatusMsg(`🔍 키워드 집계 중... (생존 ${alive.length}/${ALL_SOURCES.length}개 소스)`);

    const freq = {};
    collected.forEach(k => {
      const key = normalizeName(k.keyword);
      if (!freq[key]) freq[key] = { keyword: k.keyword, count:0, sources:[] };
      freq[key].count++;
      if (!freq[key].sources.includes(k.source)) freq[key].sources.push(k.source);
    });
    const top40 = Object.values(freq).sort((a,b) => b.count - a.count).slice(0,40);
    const keywordBlock = top40.length > 0
      ? top40.map((k,i) => `${i+1}. [${k.sources.join("+")} / 언급${k.count}회] ${k.keyword}`).join("\n")
...
1. 키워드 급상승 감지 먼저
2. 관련 상품 추출
3. 속도 데이터 분석 (총량 아님, 증가 속도만)
4. 필터: 가격 5,000~30,000원 우선 / 3초 이해 가능 / 충동구매 가능 / 2개 이상 플랫폼 / 브랜드·광고 제외
5. TOP 3만 선정
카테고리는 반드시: ${FIXED_CATEGORIES.join(", ")} 중에서만
[ 로 시작 ] 로 끝나는 순수 JSON만.
형식: [{"rank":1,"name":"상품명","category":"가전/IT","reason":"속도 근거 1문장","speedScore":85,"profitScore":78,"competitionLevel":"낮음","phase":"상승중","priceRange":"12,000~18,000원","platform":["쿠팡","네이버"],"sources":["소스명"],"frequency":3,"successFlags":{"reviewExists":true,"notExplodedYet":true,"reactionStarting":true}}]`,
          messages:[{ role:"user", content:`TOP 3 JSON만:\n\n${keywordBlock}` }]
        })
      });
      const data = await res.json();
      ...
      const msg = alive.length > 0 ? `✅ 생존 ${alive.length}개 소스 · ${top40.length}개 키워드 → TOP 3 완료` : "⚠️ RSS 전체 실패 — AI 추정 대체";
  ...
  async function fetchDetail(product) {
    ...
    if (apiSaved) {
      try {
        const nr = await fetch(`${VERCEL_URL}/api/naver`, {
          method:"POST", headers:{"Content-Type":"application/json"},
          body: JSON.stringify({ startDate:"2025-12-01", endDate:"2026-03-17", timeUnit:"week", keywordGroups:[{ groupName: product.name, keywords:[product.name] }] })
        });
        naverData = await nr.json();
      } catch {}
    }
...
형식: {"avgPrice":"12,000원","competition":"낮음","searchVolume":"월 2.1만회","searchGrowth":"+34%","coupangRating":"4.2","reviewGrowth":"+12%/주","commissionRate":"3~5%","speedTrend":[40,48,55,62,71,83,95],"shortsTip":"팁","blogTip":"팁","estimatedMonthlyRevenue":"약 18만원","entryWindow":"지금이 적기","warning":"없음"}`,
          messages:[{ role:"user", content:`"${product.name}" 분석. 네이버데이터: ${naverData ? JSON.stringify(naverData).slice(0,300) : "없음"}. 이전점수: ${prev?.speedScore || "없음"}. JSON만.` }]
        })
      });
      const data = await res.json();
      if (data.error) throw new Error(data.error.message);
      ...
      setDetail({ ...JSON.parse(txt.slice(s,e+1)), prevScore: prev?.speedScore || null, naverReal: !!naverData?.results });
  ...
  const aliveCount = healthEntries.filter(([,v])=>v.status==="alive").length;
    ...
    <div style={{ background:"#0f172a", minHeight:"100vh", fontFamily:"system-ui,sans-serif", color:"#f1f5f9" }}>
      <div style={{ background:"#1e293b", borderBottom:"1px solid #334155", padding:"14px 28px", display:"flex", alignItems:"center", gap:10 }}>
        ...
        <div>
          ...
          <div style={{ fontSize:11, color:"#64748b" }}>24~72시간 선점 · 속도가 수익을 만든다</div>
        </div>
      ...
      <div style={{ maxWidth:960, margin:"0 auto", padding:"24px 20px" }}>
        <div style={{ background:"#1e293b", borderRadius:14, padding:"6px", display:"flex", gap:4, marginBottom:20, border:"1px solid #334155" }}>
          {TABS.map(t => (
            <button key={t.id} onClick={()=>setTab(t.id)}
              style={{ flex:1, display:"flex", flexDirection:"column", alignItems:"center", gap:4, padding:"10px 8px", borderRadius:10, border:"none", cursor:"pointer",
                background:tab===t.id?"#2563eb":"transparent", color:tab===t.id?"#fff":"#64748b" }}>
            ...
            <div style={{ background:"#1e293b", borderRadius:16, padding:24, border:"1px solid #334155" }}>
              <div style={{ display:"flex", alignItems:"center", justifyContent:"space-between", marginBottom:16 }}>
                <div>
                  ...
                  <div style={{ fontSize:12, color:"#475569", marginTop:4 }}>{lastFetched ? `마지막 수집: ${lastFetched.toLocaleTimeString("ko-KR")}` : "키워드 속도 기반 분석"}</div>
                </div>
              ...
              {healthEntries.length > 0 && (
                <div style={{ background:"#0f172a", border:"1px solid #1e293b", borderRadius:10, padding:"10px 14px", marginBottom:16 }}>
                  <div style={{ fontSize:11, fontWeight:700, color:"#475569", marginBottom:8 }}>📡 소스 생존 현황 · 생존 {aliveCount}/{ALL_SOURCES.length}</div>
                  <div style={{ display:"flex", flexWrap:"wrap", gap:6 }}>
                    {healthEntries.map(([name,v])=>(
                      <span key={name} style={{ background:v.status==="alive"?"#052e16":v.status==="dead"?"#1f0707":"#1c1507", border:`1px solid ${v.status==="alive"?"#16a34a":v.status==="dead"?"#dc2626":"#d97706"}`, borderRadius:6, padding:"2px 8px", fontSize:11, color:v.status==="alive"?"#4ade80":v.status==="dead"?"#f87171":"#fbbf24", fontWeight:600 }}>
                        {v.status==="alive"?"✅":v.status==="dead"?"❌":"⏳"} {name}{v.status==="alive"&&v.count>0&&<span style={{opacity:0.6}}> {v.count}건</span>}
                      </span>
                    ))}
                  </div>
                </div>
              ...
              {!loading && products.length===0 && !statusMsg && (
                <div style={{ background:"#1c150740", border:"1px solid #d97706", borderRadius:10, padding:"14px 16px", fontSize:13, color:"#fbbf24" }}>
                  💡 <b>지금 헌팅</b> 버튼을 눌러 TOP 3를 발굴하세요
                </div>
              )}
              {loading && <div style={{ textAlign:"center", padding:"50px 0", color:"#475569" }}><div style={{ fontSize:40, marginBottom:12 }}>🚀</div><div>소스 확인 → 수집 → AI 분석 중...</div></div>}
                      ...
                      <div style={{ display:"flex", alignItems:"flex-start", gap:12 }}>
                        ...
                        <div style={{ flex:1 }}>
                          ...
                          <div style={{ display:"flex", gap:6, flexWrap:"wrap" }}>
                            ...
                            {p.frequency>1&&<span style={{ background:"#1e40af20", border:"1px solid #3b82f6", borderRadius:6, padding:"2px 8px", fontSize:11, color:"#60a5fa", fontWeight:600 }}>🔁 {p.frequency}개 소스</span>}
                          </div>
                        </div>
        ...
        {tab===1 && (
          <div style={{ background:"#1e293b", borderRadius:16, padding:24, border:"1px solid #334155" }}>
            ...
            {detailLoading&&<div style={{ textAlign:"center", padding:"50px 0", color:"#475569" }}><div style={{ fontSize:36, marginBottom:10 }}>🔎</div><div>분석 중...</div></div>}
            {detail&&!detail.error&&(
              <div>
                <div style={{ background: detail.naverReal?"#052e1640":"#1c150740", border:`1px solid ${detail.naverReal?"#16a34a":"#d97706"}`, borderRadius:8, padding:"8px 14px", marginBottom:16, fontSize:12, color:detail.naverReal?"#4ade80":"#fbbf24" }}>
                  {detail.naverReal ? "✅ 네이버 실제 데이터 기반" : "⚠️ AI 추정값 · 네이버 API 연동 후 실제 데이터 교체"}
                </div>
